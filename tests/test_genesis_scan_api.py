...
def test_collect_scan_table_returns_one_row_per_case(tmp_path: Path) -> None:
    ...
    assert "stat_max_energy" in df.columns
    assert "stat_max_power" in df.columns
    assert "stat_max_ppower" in df.columns
    assert "stat_energy@1.00m" in df.columns
    assert df.loc[df["case_id"] == "1", "stat_max_energy"].item() == 2.2e-6
    assert df.loc[df["case_id"] == "2", "stat_max_energy"].item() == 2.8e-6
...
def test_collect_scan_rows_support_metric_subset_and_column_order(tmp_path: Path) -> None:
    ...
    assert rows == [{"case_id": "1", "stat_max_energy": 2.2e-6, "stat_energy@1.00m": 2.2e-6}]
...
def test_collect_scan_table_supports_custom_result_relpath(tmp_path: Path) -> None:
    ...
    assert df["case_id"].tolist() == ["1"]
    assert df["stat_max_energy"].tolist() == [2.2e-6]


def test_collect_scan_rows_keeps_param_named_like_a_metric(tmp_path: Path) -> None:
    _write_case(
        tmp_path,
        directory="case001",
        power=np.array([[1.0, 3.0, 2.0], [2.0, 5.0, 4.0]]),
    ...
    (tmp_path / "cases.csv").write_text(
        "case_id,directory,max_energy\n"
        "1,case001,999\n",
        encoding="utf-8",
    )

    rows = collect_scan_rows(tmp_path, metric_names=["max_energy"])

    assert rows == [{"case_id": "1", "max_energy": 999, "stat_max_energy": 2.2e-6}]
